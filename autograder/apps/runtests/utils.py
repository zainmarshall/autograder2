...
class SubmissionQueue:
    ...
    async def _run(self):
        ...
        while not self._paused:
            try:
                ...
                logger.exception(
                    f"An error occurred while processing submission ID {self._currently_processing}: {e}"
                )
    ...
    async def _grade_submission(self, submission_id: int):
        ...
        coderunner_url = (
            settings.CODERUNNER_URL
        )  # Recommended: Store URL in settings.py
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.info(
                        f"Response from coderunner for submission {submission_id}: {data}"
                    )
            ...
            await self._mark_submission_as_error(
                submission_id, f"Could not connect to the grading service: {e}"
            )
        except Exception as e:
            logger.exception(
                f"An unexpected error occurred during grading for submission {submission_id}."
            )
            await self._mark_submission_as_error(
                submission_id, f"An unexpected grading server error occurred: {e}"
            )

    @staticmethod
    @sync_to_async
    def update_submission_from_result(
        submission: Submission, result_data: Dict[str, Any]
    ):
    ...
    async def _mark_submission_as_error(self, submission_id: int, error_message: str):
        ...
        try:
            ...
            logger.error(
                f"Could not find submission {submission_id} to mark as an error."
            )
        except Exception as e:
            logger.error(
                f"Failed to save error state for submission {submission_id}: {e}"
            )
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        return {
            "is_running": self._runner_task is not None
