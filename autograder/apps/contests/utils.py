...
def get_standings(cid):
    ...
    for s in subs:
        ...
        if s.verdict in ('Accepted', 'AC'):
            if user_data['problems'][prob_idx] == 0:
                ...
                user_data['penalty'] += minutes - 10 * abs(min(0, user_data['problems'][prob_idx]))
    ...
    # Filter and sort
    standings = [u for u in stats.values()
                 if u['solved'] > 0 and u['penalty'] >= 0 and u['id']]
    ...
    for idx, row in enumerate(standings, start=1):
        if prev and row['solved'] == prev['solved'] and row['penalty'] == prev['penalty']:
            row['rank'] = prev['rank']
        else:
            row['rank'] = idx
        prev = row
